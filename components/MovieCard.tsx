...
const MovieCard = ({
    id, poster_path, title, vote_average, release_date, vote_count, adult
}: Movie ) => {
  return (
   <Link href={`/movies/${id}`} asChild>
    ...
    <TouchableOpacity className='w-[30%]'>
        ...
        <View className='flex-row items-center justify-start gap-x-1'>
            ...
            <Text className='text-xs text-light-300 font-bold uppercase'>({Intl.NumberFormat('en', { notation: 'compact', compactDisplay: 'short' }).format(vote_count)})</Text>
        </View>

        <View className='flex-row items-center justify-between'>
            <View className='flex-row gap-x-2'>
                <Text className='text-xs font-medium text-light-300 uppercase'>Movie</Text>
                {/* for the future | add movie category name  */}
            </View>
